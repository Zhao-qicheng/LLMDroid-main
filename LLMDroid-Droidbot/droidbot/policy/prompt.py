...
The value of "Target Function" must be copied exactly from the corresponding State's FunctionList.
...
You will also receive a Candidate Actions JSON array. You must choose only one existing (element_id, action_type) pair from that array. If none of the candidate actions can advance or complete the target function, return {"Element Id": -1, "Action Type": 0}.
"""
...
The value of "Element Id" must be an integer number copied from an HTML id attribute, or -1. Do not use resource-id, text, content-desc, function names, or labels as "Element Id".
...
The value of "Action Type" must be an integer number from 0 to 6.
The pair of "Element Id" and "Action Type" must appear in the Candidate Actions JSON array as element_id and action_type.
