...
class InputEvent(object):
    ...
    def __init__(self, action_type: ActionType = ActionType.OTHER):
        ...
        self.log_lines = None
        self.action_type: ActionType = action_type  # LLMDroid's attribute
    ...
    def to_description(self) -> str:
        return f"Action {self.action_type.string}"
...
class EventLog(object):
    ...
    def save2dir(self, output_dir=None):
        ...
        try:
            ...
            json.dump(self.to_dict(), event_json_file, indent=2, default=utils.custom_serializer)
...
class ManualEvent(InputEvent):
    ...
    def to_description(self) -> str:
        return f"Manual Event: {self.get_event_str(None)}"
...
class UIEvent(InputEvent):
    ...
    def __hash__(self):
        hashcode1 = hash(self.action_type.string)
        hashcode2 = hash(self._target) if self._target else 0x1
...
class TouchEvent(UIEvent):
    ...
    def __init__(self, x=None, y=None, view=None, event_dict=None):
        super().__init__(ActionType.CLICK, widget=utils.safe_dict_get(view, 'widget', None))
...
class SelectEvent(UIEvent):
    ...
    def __init__(self, event_type=KEY_SelectEvent, x=None, y=None, view=None, event_dict=None):
        super().__init__(ActionType.LONG_CLICK, widget=utils.safe_dict_get(view, 'widget', None))
    ...
    def get_event_str(self, state):
        if self.view is not None:
            ...
            return "%s(type=%s, state=%s, x=%s, y=%s)" % (
            self.event_type, self.__class__.__name__, state.state_str, self.x, self.y)
        else:
            msg = "Invalid %s!" % self.__class__.__name__
            raise InvalidEventException(msg)
...
class ScrollEvent(UIEvent):
    ...
    def to_description(self, html: str = '') -> str:
        ...
        elif self.action_type == ActionType.SCROLL_LEFT_RIGHT:
            description += " from left to right"
        elif self.action_type == ActionType.SCROLL_RIGHT_LEFT:
...
class SetTextEvent(UIEvent):
    ...
    def __init__(self, x=None, y=None, view=None, text=None, event_dict=None):
        super().__init__(ActionType.INPUT, widget=utils.safe_dict_get(view, 'widget', None))
